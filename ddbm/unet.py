...
from diffae.templates_latent import *
...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=512, nc=3, im_size=128):
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.end=end
        if sto:
            self.num_classes = latent_dim *2
        else:
            self.num_classes = latent_dim

        print("latent_dim in unet:", latent_dim)
        print("sto:",sto)
        print("kl:",kl)
        print("stoxt",stoxt)
        ...
        if self.num_classes is not None:
            self.label_emb = linear(latent_dim, time_embed_dim)
        ...
        if image_size == 128:
            conf_ = ffhq128_autoenc_130M()
        elif image_size == 64:
            conf_ = celeba64d2c_autoenc()
        else:
            assert 0
        conf = conf_.make_model_conf()
        encoder = BeatGANsEncoderConfig(
        ...
        if self.end:
            ...
            self.decoder = Generator(nz=latent_dim, im_size=image_size)
    ...
    def encode(self, x0, out_all=False):
        if self.sto:
            out = self.encoder(x0)
            mu_, log_sig_ = torch.chunk(out,2,dim=1)
            mu = soft_clamp5(mu_)
            log_sig = soft_clamp5(log_sig_)
