...
def load_data(
  ...
  if dataset == 'ffhq':
      from .aligned_dataset import FFHQ
      trainset = FFHQ(dataroot=root, train=True, img_size=image_size,augmentation=True)
      valset = FFHQ(dataroot=root, train=True, img_size=image_size, augmentation=False)
      if include_test:
          testset = FFHQ(dataroot=root, train=False, img_size=image_size, augmentation=False)

  elif dataset == 'celeba':
      from .aligned_dataset import CELEBA64
      trainset = CELEBA64(dataroot=root, split="train", img_size=image_size,augmentation=True)
      valset = CELEBA64(dataroot=root, split="train", img_size=image_size, augmentation=False)
      if include_test:
          testset = CELEBA64(dataroot=root, split="test", img_size=image_size, augmentation=False)

  elif dataset == 'horse':
      from .aligned_dataset import HORSE
      trainset = HORSE(dataroot=root, train=True, img_size=image_size,augmentation=True)
      valset = HORSE(dataroot=root, train=True, img_size=image_size, augmentation=False)
      if include_test:
          testset = HORSE(dataroot=root, train=False, img_size=image_size, augmentation=False)

  elif dataset == 'bedroom':
      from .aligned_dataset import BEDROOM
      trainset = BEDROOM(dataroot=root, train=True, img_size=image_size,augmentation=True)
      valset = BEDROOM(dataroot=root, train=True, img_size=image_size, augmentation=False)
      if include_test:
          testset = BEDROOM(dataroot=root, train=False, img_size=image_size, augmentation=False)

  elif dataset == 'celebahq':
      from .aligned_dataset import CELEBAHQ
      trainset = CELEBAHQ(dataroot=root, train=True, img_size=image_size,augmentation=True)

      valset = CELEBAHQ(dataroot=root, train=True, img_size=image_size, augmentation=False)
      if include_test:
          testset = CELEBAHQ(dataroot=root, train=False, img_size=image_size, augmentation=False)
