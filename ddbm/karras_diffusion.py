...
from ddbm.losses import normal_kl
...
class KarrasDenoiser:
    def __init__(
        ...
        self.sigma_data_end = end_data
        print("self.sigma_data_end:",self.sigma_data_end)
        self.cov_xy = cov_xy
        self.c = 1
    ...
    def training_bridge_losses(self, model, x_start, sigmas, model_kwargs=None, noise=None, vae=None, kl=False,end=False):
        
        assert model_kwargs is not None

        ## Encode
        z = model.module.encode(x_start)
        ## Decode
        if end:
            model_kwargs['xT'] = model.module.decode(z)
        xT = model_kwargs['xT']
        ...
        model_output, denoised = self.denoise(model, x_t, sigmas,z,  **model_kwargs)
    ...
    def denoise(self, model, x_t, sigmas, y ,**model_kwargs):
        ...
        model_output = model(c_in * x_t, rescaled_t, y=y, **model_kwargs)
...
def karras_sample(
    ...
    assert sampler in ["heun", "euler"], 'only heun sampler is supported currently'

    sigmas = get_sigmas_karras(steps, sigma_min, sigma_max-1e-4, rho, device=device)

    sample_fn = {
        "heun": partial(sample_heun, beta_d=diffusion.beta_d, beta_min=diffusion.beta_min, type=sampler),
    }["heun"]

    sampler_args = dict(
            pred_mode=diffusion.pred_mode, churn_step_ratio=churn_step_ratio, sigma_max=sigma_max
        )
    def denoiser(x_t, sigma,y, x_T=None):
        model_kwargs['xT'] = x_T
        _, denoised = diffusion.denoise(model, x_t, sigma, y, **model_kwargs)
...
def sample_heun(
    ...
    for j, i in enumerate(indices):
        ...
        if churn_step_ratio > 0:
            ...
            denoised = denoiser(x, sigmas[i] * s_in, y, x_T)
            if pred_mode == 've':
        ...
        # heun step
        denoised = denoiser(x, sigma_hat * s_in, y, x_T)
        if pred_mode == 've':
        ...
        #dt = sigmas[i - 1] - sigma_hat
        #print("sigmas[i + 1], sigma_hat",sigmas[i + 1],sigma_hat)

        if sigmas[i + 1] == 0 or type=="euler":
            x = x + d * dt
            
        elif type=="heun":
            # Heun's method
            x_2 = x + d * dt
            denoised_2 = denoiser(x_2, sigmas[i + 1] * s_in,y, x_T)
            if pred_mode == 've':
